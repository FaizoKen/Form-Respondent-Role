...
RUN mkdir src && echo "fn main() {}" > src/main.rs && \
    cargo build --release && \
    rm -rf src target/release/form-respondent-role target/release/deps/form_respondent_role*
...
COPY --from=builder /app/target/release/form-respondent-role /usr/local/bin/
...
HEALTHCHECK --interval=15s --timeout=3s --start-period=10s --retries=3 \
    CMD curl --fail --silent --max-time 2 \
        http://127.0.0.1:8089/form-respondent-role/health || exit 1

USER app:app

CMD ["form-respondent-role"]
