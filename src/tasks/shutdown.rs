...
//!   * every job-polling worker (so it finishes the in-flight batch and exits)
//!   * the rate-limiter GC and any other long-running tasks
//!
//! After `wait_until_triggered()` resolves, callers should NOT start new work
//! but MUST finish what's already in flight. The main loop in `main.rs` joins
//! all worker handles before exiting so no in-flight job is silently dropped.
