...
//!
//! The dashboard config UI uses `ui_mode: "iframe"` — RoleLogic embeds the
//! plugin's own role-config page (`/admin/{guild}/role/{role_id}`) in an
//! iframe and the plugin owns the entire save UX. POST /config is a no-op
//! kept around for contract compliance: it is never called by iframe-mode
//! plugins, but we still verify the token so a stale call cannot ping us
//! silently.
...
/// POST /config is a no-op: the dashboard schema is read-only, real edits
/// happen on the plugin web. We still verify the token so a leaked/stale
/// token can't ping us silently. Extra fields in the body are ignored.
#[derive(Deserialize)]
