...
pub async fn favicon() -> impl IntoResponse {
    ...
    // days. Saves a re-fetch on every page load for repeat visitors.
    // Switch to a content-hashed filename (`favicon.{hash}.ico`) if you
    // ever need to cache-bust without waiting out the window.
    (
        [
