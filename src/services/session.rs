...
mod tests {
    ...
    fn verify_rejects_id_swap() {
        // An attacker can read a cookie for user A and try to substitute the
        // discord_id portion to impersonate user B. Without changing the sig
        // this MUST fail.
    ...
    fn verify_rejects_malformed() {
        ...
        assert!(verify_session("a:b:c", SECRET).is_none()); // 3 parts, need 4
