...
mod tests {
    ...
    fn rejects_subdomain_of_allowed() {
        // app.rolelogic.com is allowed; attacker.rolelogic.com is NOT a
        // substring/suffix match — must be exact-origin.
    ...
    fn rejects_path_in_origin_value() {
        // Origin header should never include a path, but if it does the
        // comparison must not naively pass on a prefix match.
